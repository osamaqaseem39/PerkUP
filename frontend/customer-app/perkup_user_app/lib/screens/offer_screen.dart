...
class OfferScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Fetch perks with PerkTypeID = 3 (Offers) on screen load
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final token = loginProvider.token;
      if (token != null) {
        perkProvider.fetchPerksByType(3, token); // PerkTypeID for Offers
      }
    });
        ...
        builder: (context, perkProvider, child) {
                      ...
                      onPressed: () {
                        final token =
                            Provider.of<LoginProvider>(context, listen: false)
                                .token;
                        if (token != null) {
                          perkProvider.fetchPerksByType(3, token);
                        }
                      },
          ...
          return RefreshIndicator(
            onRefresh: () async {
              ...
              if (token != null) {
                await perkProvider.fetchPerksByType(3, token);
              }
            },
