...
const CountryList = () => {
  ...
  const handleEditClick = (countryID: number) => {
    navigate(`/countryform/${countryID}`);
  };

  const handleAddNewClick = () => {
    navigate('/countryform');
  };

  const handleDeleteClick = (countryID: number) => {
  ...
  const handleConfirmDelete = async () => {
    if (countryToDelete !== null) {
      try {
        await api.delete(`/Countries/${countryToDelete}`);
        setCountries(countries.filter((country) => country.countryID !== countryToDelete));
      } catch (error) {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="border-b border-stroke py-4 px-6.5 dark:border-strokedark flex justify-between items-center">
        <h2 className="font-medium text-2xl text-black dark:text-white">Countries</h2>
        <button
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              <th className="min-w-[220px] py-4 px-4 font-medium text-black dark:text-white xl:pl-11">Country Name</th>
            ...
            {countries.map((country) => (
              <tr key={country.countryID}>
                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                  <h5 className="font-medium text-black dark:text-white">{country.countryName}</h5>
                </td>
