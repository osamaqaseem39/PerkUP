...
const MenuForm = () => {
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            setMenu({
                ...menu,
                image: URL.createObjectURL(e.target.files[0]) // Temporarily display the image
            });
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            formData.append('isActive', menu.isActive ? 'true' : 'false');
            // Append menu items
            (menu.menuItems || []).forEach((item, index) => {
                formData.append(`menuItems[${index}].itemName`, item.itemName || '');
                ...
                formData.append(`menuItems[${index}].discount`, item.discount.toString());
                formData.append(`menuItems[${index}].isPercentageDiscount`, item.isPercentageDiscount.toString());
                formData.append(`menuItems[${index}].isActive`, item.isActive.toString());
                formData.append(`menuItems[${index}].category`, item.category || '');
                formData.append(`menuItems[${index}].createdBy`, item.createdBy.toString());
                formData.append(`menuItems[${index}].createdAt`, item.createdAt || '');
                formData.append(`menuItems[${index}].updatedBy`, item.updatedBy.toString());
                formData.append(`menuItems[${index}].updatedAt`, item.updatedAt || '');
            });
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5.5 p-6.5">
                ...
                <div>
                    ...
                    <input
                        ...
                        className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                    />
                    {menu.image && <img src={menu.image} alt="Menu preview" className="mt-2 w-32 h-32 object-cover" />}
                </div>
                <div>
                    ...
                    <input
                        ...
                        onChange={() => setMenu({ ...menu, isActive: !menu.isActive })}
